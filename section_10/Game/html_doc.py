...
class Body(Tag):
    ...
    def __init__(self):
        super().__init__('body', '')  # body contents be built up separately
        self._body_contents = []
...
if __name__ == '__main__':
    # my_page = HtmlDoc("My awesome title goes here")
    ...
    new_body = Body()
    new_body = Body()
    new_body.add_tag('h1', 'Aggreagation')
    new_body.add_tag('p', "Unlike <strong>composition</strong>, aggreagation uses existing instances"
    ...
    new_body.add_tag('p', "The composed object doesn't actually own the object that it's composed of"
                          " - if it's destoryed, those objects continue to exist.")
